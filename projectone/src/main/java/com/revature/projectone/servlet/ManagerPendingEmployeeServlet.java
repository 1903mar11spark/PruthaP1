...
public class ManagerPendingEmployeeServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
	ReimbursementDAOImpl theDao = new ReimbursementDAOImpl();

    public ManagerPendingEmployeeServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (session !=null && session.getAttribute("username")!= null) {
			try {
				...
				reimbs =theDao.getAllPendingMan(employeeId);
				...
				e.printStackTrace();
				response.getWriter().write("{\"manaerviewpending\":null}");
				System.out.println(session + "managerviewpending");
			} 
		}else {
				response.getWriter().write("{\"managerviewpending\":null}");
				System.out.println(session + "managerviewpending");
		}
